...
function init_rheologies(layers, oxd_wt, fric_angle; linear = false, incompressible = true, plastic = true, magma = false, softening_C=true, softening_ϕ=false)
    ...
    pl_seed = DruckerPrager_regularised(; C = 1e6, softening_C = soft_C, ϕ=5.0, η_vp=η_reg, Ψ=0.0, ) # regularized plasticity
    ...
    return rheology = (
        ...
        SetMaterialParams(;
            ...
            CompositeRheology = CompositeRheology((disl_top,el,pl_seed)),
            RadioactiveHeat   = ConstantRadioactiveHeat(),
            ShearHeat         = ConstantShearheating(1.0NoUnits),
            Gravity           = ConstantGravity(; g=9.81),
        ),
