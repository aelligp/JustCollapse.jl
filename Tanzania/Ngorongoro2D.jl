...
include("Ngorongoro_setup.jl")
include("Ngorongoro_rheology.jl")
...
function main(li, origin, phases_GMG, T_GMG, T_bg, igg; nx = 16, ny = 16, figdir = "figs2D", do_vtk = false, fric_angle = 30, extension = 1.0e-15 * 0, cutoff_visc = (1.0e16, 1.0e23), V_total = 0.0, V_eruptible = 0.0, layers = 1, air_phase = 6, progressiv_extension = false, plotting = true, displacement=false)
    ...
    nxcell, max_xcell, min_xcell = 100, 125, 75
    ...
    compute_cells_for_Q!(cells, 0.01, phase_ratios, 5 ,6, ϕ_m);
    V_total = compute_total_eruptible_volume(cells, di...);

    tmax = 2.5e6 * 3600 * 24 * 365.25
    ...
    while t < tmax
        ...
        if it > 3
            CUDA.@allowscalar pp = [p[5] > 0 || p[6] > 0 for p in phase_ratios.center]
            # pp = [p[5] > 0 || p[6] > 0 for p in phase_ratios.center]
            compute_cells_for_Q!(cells, 0.01, phase_ratios, 5 ,6, ϕ_m)
            ...
            if eruption == false && !isempty(Array(stokes.P)[pp][Array(ϕ_m)[pp] .≥ 0.3]) &&
                ...
                compute_cells_for_Q!(cells, 0.5, phase_ratios, 5 ,6, ϕ_m)
                V_tot = V_total
                if (V_total - V_erupt) > 0.0
                    ...
                    V_total, V_erupt = make_it_go_boom_smooth!(stokes.Q, cells, ϕ_m, V_erupt, V_tot, weights, phase_ratios, 5 ,6)
                end
            ...
            elseif eruption == false && !isempty(Array(stokes.P)[pp][Array(ϕ_m)[pp] .≥ 0.3]) &&
                ...
                @views stokes.Q .= 0.0
                @views thermal.H .= 0.0
                compute_cells_for_Q!(cells, 0.3, phase_ratios, 5 ,6, ϕ_m)
                ...
                V_total, V_erupt = make_it_go_boom_smooth!(stokes.Q, cells, ϕ_m, V_erupt, V_tot, weights, phase_ratios, 5 ,6)
                compute_thermal_source!(thermal.H, T_addition, 0.3, V_erupt, V_tot, ϕ_m, phase_ratios, dt, args, di,  5 ,6, rheology, cells)
...
# figdir   = "ProjectTanzania_Ngorongoro/Ngorongoro2D_$(today())_granite_d_$(depth)_r_$(radius)_ar_$(ar)_ex_$(extension)_phi_$(fric_angle)"
figdir = "ProjectTanzania_Ngorongoro/Ngorongoro2D_$(today())"
n = 256
nx, ny = n, n >> 1
...
li, origin, phases_GMG, T_GMG, T_bg, _, V_total, V_eruptible, layers, air_phase = setup2D(
    ...
    volcano_location = (30.0, 0.0), # location of the volcano center (x,y)
    chamber_T = 975.0e0, # temperature of the chamber
    chamber_depth = 5.0e0,
...
main(li, origin, phases_GMG, T_GMG, T_bg, igg; figdir = figdir, nx = nx, ny = ny, do_vtk = do_vtk, fric_angle = fric_angle, cutoff_visc = (1.0e17, 1.0e23), V_total = V_total, V_eruptible = V_eruptible, layers = layers, air_phase = air_phase, progressiv_extension = progressiv_extension, plotting = plotting, displacement=displacement);
