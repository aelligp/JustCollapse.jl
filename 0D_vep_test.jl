...
# melt_factor = zeros(size(ϕ))

# ϕ_f, ϕ_s = 1.0, 0.0

# @parallel function compute_melt_factor!(melt_factor, ϕ, S, mfac, η_f, η_s)
#     @all(melt_factor) = max(η_f*(1.0 - S * (1.0 - @all(ϕ)))^mfac, η_s)
#     return nothing
# end


# @parallel compute_melt_factor!(melt_factor, ϕ, S, mfac, ϕ_f, ϕ_s)
...
function DikeInjection_2D(igg; figname=figname, nx=nx, ny=ny)
    ...
    η = MatParam[1].CompositeRheology[1][1].η.val       # viscosity for the Rayleigh number
    ...
    k0 = MatParam[1].Conductivity[1]              # Conductivity
    ...
    args = (; ϕ=ϕ, T=thermal.Tc, P=stokes.P, dt=Inf, pressure_top=pressure_top, sticky_air=mask_sticky_air)
    ...
    while it < 15 #nt
        ...
        if it == 1 || rem(it, 1) == 0
            ...
            if igg.me == 0
                ...
                # p2 = heatmap!(ax2, x_c, y_c, P_d; Colormap=:roma)
