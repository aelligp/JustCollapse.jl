...
@views function DikeInjection_2D(igg; figname=figname,nx=nx, ny=ny); 
    ...
    Model3D_Cross_ni              = CrossSection(Model3D_cart, dims=(ni[1],ni[2]), Interpolate=true,Start=(ustrip(-50.00km),ustrip(60.00km)), End=(ustrip(-06.00km), ustrip(20.00km)));
    # Model3D_Cross              = CrossSection(Model3D_cart, dims=(ni[1]+1,ni[2]+1), Interpolate=true,Start=(ustrip(-10.00km),ustrip(10.00km)), End=(ustrip(-06.00km), ustrip(00.00km)));
    Model3D_new_ni            = FlattenCrossSection(Model3D_Cross_ni);
    Model3D_Cross_ni           = AddField(Model3D_Cross_ni,"Model3D_Cross",Model3D_new);
    ...
    v_extension             = nondimensionalize(1e-15m/s, CharDim)   # extension velocity for pure shear boundary conditions
    ...
    if toy == true
        xc, yc      = 0.68*lx, 0.2*lz  # origin of thermal anomaly
        #Pressure Gaussian 
       ...
       @show dt
        ...
        if it == 1 || rem(it, 1) == 0
            ...
            if igg.me == 0
                ...
                ax4 = Axis(fig[3,2][1,1], aspect = ar, title = L"\varepsilon_{\textrm{II}}[\mathrm{s}^{-1}]", xlabel="Width [km]", titlesize=40, yticklabelsize=25, xticklabelsize=25, xlabelsize=25)
...
function run()
    figname = "extension_Toba_Inversion_GeoMod"
    # nx     = 384-3    #to include ghost nodes with optimal performance at a multiple of 32
    # ny     = 768-3    #to include ghost nodes
    nx     = 224    #to include ghost nodes with optimal performance at a multiple of 32
    ny     = 224    #to include ghost nodes
