...
if USE_GPU  model = PS_Setup(:gpu, Float64, 2)            # initialize parallel stencil in 2D
            environment!(model)      
...
@views function DikeInjection_2D(); 
    ...
    if isdir("./Paraview/new_solver")==false mkdir("./Paraview/new_solver") end; loadpath = "./Paraview/new_solver/"; 
    ...
    while it < nt
       ...
       @show dt
        ...
        if it == 1 || rem(it, 1) == 0
            ...
            fig = Figure(resolution = (2000, 2000), createmissing = true, title="t = $(t_Myrs/1e3) Kyrs")
            ar = 2.0
            ar = DataAspect()

            ax0 = Axis(fig[1,1:2], aspect = ar,title="t = $((ustrip.(t_Kyrs))) Kyrs", titlesize=30, height=0.0)
            ...
            ax2 = Axis(fig[2,2][1,1], aspect = ar, title = L"\log_{10}(\eta [\mathrm{Pas}])",titlesize=20)
            ax3 = Axis(fig[3,1][1,1], aspect = ar, title = L"Vy [\mathrm{cm/yr}]", titlesize=20)
            ax4 = Axis(fig[3,2][1,1], aspect = ar, title = L"\rho [\mathrm{kgm}^{-3}]", xlabel="Width [km]", titlesize=20)
            ax5 = Axis(fig[4,1][1,1], aspect = ar, title = L"\phi", xlabel="Width [km]", titlesize=20)
           ...
           hidexdecorations!(ax4, grid = false)
            ...
            p5  = heatmap!(ax5, x_c, y_c, ϕ_d, colormap=:lajolla,xlims=(20.0,55.0), ylims=(-20.0, maximum(y_v)))
