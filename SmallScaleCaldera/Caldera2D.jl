...
function main(li, origin, phases_GMG, T_GMG, T_bg, igg; nx = 16, ny = 16, figdir = "figs2D", do_vtk = false, fric_angle = 30, extension = 1.0e-15 * 0, cutoff_visc = (1.0e16, 1.0e23), V_total = 0.0, V_eruptible = 0.0, layers = 1, air_phase = 6, progressiv_extension = false, plotting = true, displacement=false)
    ...
    # pt_stokes = PTStokesCoeffs(li, di;  ϵ_abs = 1.0e-4, ϵ_rel = 1.0e-4, Re = π / 2, r = 0.7, CFL = 0.98 / √2.1) # Re=3π, r=0.7
    pt_stokes = PTStokesCoeffs(li, di; ϵ_rel = 1.0e-4, ϵ_abs = 1.0e-4, Re = 3.1, r = 0.7, CFL = 0.8 / √2.1) # Re=3π, r=0.7
    # pt_stokes = PTStokesCoeffs(li, di; ϵ_rel = 1.0e-4, ϵ_abs = 1.0e-4, Re = 14.9, r = 0.7, CFL = 0.8 / √2.1) # Re=3π, r=0.7
    # pt_stokes = PTStokesCoeffs(li, di; ϵ_rel = 1.0e-5, ϵ_abs = 1.0e-4, CFL = 0.98 / √2.1) # Re=3π, r=0.7
    ...
    while it < 150
        ...
        if eruption == true
            er_it += 1
            dtmax = min(dt_diff, 100 * 3600 * 24 * 365.25)
        elseif it < 3
            dtmax = 1e3 * 3600 * 24 * 365.25
        else
            # dtmax = min(dt_diff, 1e3 * 3600 * 24 * 365.25)
            dtmax = 1e3 * 3600 * 24 * 365.25
        end
        ...
        if plotting
            ...
            if it == 1 || rem(it, 1) == 0
                ...
                fig1 = let
                    ...
                    s1 = scatter!(ax, Array((stokes.P) ./ 1.0e6)[:], Array(stokes.τ.II ./ 1.0e6)[:]; color = Array(pp)[:], colormap = cgrad(:roma, length(rheology), categorical = true), markersize = 3)
                    Colorbar(fig[1, 2], s1)
                    fig
...
figdir = "Systematics/Dim_Caldera2D_$(today())"
n = 128
nx, ny = n, n >> 1
