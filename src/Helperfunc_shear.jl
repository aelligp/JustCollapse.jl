...
@parallel_indices (i, j)  function init_P_topo!(P, ρg, z, phase)
    if phase[i,j] <3
        @all(P) = @all(ρg)*abs(@all_j(z))
    else
        @all(P) = 0.0
...
@parallel_indices (i, j) function init_P_topography1!(P, ρg, z,phase)
    @inbounds if phase[i,j] < 3 
        P[i, j] = (ρg[i, j] * (-z[j])) #+ maximum((ρg[i, j] * (-z[j])))
             #- max(P[i, j], z[j] < 0)
    else
        P[i, j] = nondimensionalize(0.0MPa,CharDim)
    end
    return nothing
...
# @parallel_indices (i, j) function init_P_topography1!(P, ρg, z,phase)
#     @inbounds if phase[i,j] < 3 
#         P[i, j] = [(-z[iy] * ρg[ix,iy]) .+ max.(abs(-z[iy]) * ρg[ix,iy]) for ix in axes(P,1), iy in axes(P,2)]
#     else
#         P[i, j] = 0.0
#     end
#     return nothing
